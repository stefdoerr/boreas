# Boreas — developer guide

Building from source, installing local builds, cross-compiling for the MOD Dwarf, the
release / publishing workflow, and how the DSP works. **Using Boreas needs none of
this** — see the [README](README.md#install) to download a prebuilt bundle.
...
| `./install.sh` | Install into MOD Desktop's user-plugin dir (`BETA=1 ./install.sh` for the beta) |
| `make install` | Copy the LV2 to `/usr/lib/lv2/` (use `sudo`) |
...
The `dwarf-*` targets need Docker; the one-time `make dwarf-image` builds the aarch64
cross-toolchain image (~30–60 min). Plain `make` builds all three desktop formats
(LV2 + VST3 + CLAP) with your host toolchain — copy the `.vst3` / `.clap` into `~/.vst3`
/ `~/.clap` to test in a DAW. The identity (name/brand/URI) lives in the top-level
[`Makefile`](Makefile) and [`DistrhoPluginInfo.h`](plugins/Boreas/DistrhoPluginInfo.h).
...
`make release version=x.y.z` builds the MOD / Patchstorage LV2 bundles, the Dwarf
build, and the PDF manual, tags the commit, pushes, and creates the GitHub release.
GitHub Actions then cross-builds the desktop **VST3 / CLAP** for Linux, Windows, and
macOS (`.github/workflows/desktop-release.yml`, via DISTRHO's `dpf-makefile-action`)
and attaches them to the same release — macOS can't be built on a Linux machine, so
CI is their natural home.

**MOD Dwarf and Patchstorage bundles stay local.** Their cross-toolchains (the Dwarf
image, the Patchstorage platform images) take ~30–60 min to assemble and are hard to
cache on a fresh runner, whereas locally the images are already there and a build is
~10 s. So one `make release` creates the release with the local bundles, and CI
appends the desktop VST3/CLAP to it.
